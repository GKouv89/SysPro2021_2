...
ΠΡΩΤΟΚΟΛΛΟ ΕΠΙΚΟΙΝΩΝΙΑΣ
    ...
    2) Αρχικοποίηση
        2.1) Το μέγεθος του buffer, καθώς και το μέγεθος των bloomfilters μεταφέρεται στο παιδί ως int. Μετά, μεταφέρεται το
            ...
            και μόνο αφού ληφθεί ολόκληρο το όνομα του φακέλου στέλνεται επιβεβαίωση στον γονέα. Αφού ο γονιός διαβάσει τα 
        ...
        2.2) Ο γονιός δημιουργεί ένα file descriptor set με όλα του τα reading file descriptors και μπλοκάρει με την select.
            Όταν η select επιστρέψει, διατρέχει όλα τα file descriptors στο set, και για κάθε ένα διαθέσιμο για ανάγνωση,
    ...
    3) Εντολές
        ...
        3.2) /travelRequest, περίπτωση όπου πρέπει να ελέγξουμε το skiplist ενός ιού.
            Ο γονιός απευθύνει στο Monitor process ένα ερώτημα της μορφής: checkSkiplist citizenID virusName countryFrom. Στέλνει το μήκος του ερωτήματος και 
            κατόπιν το ίδιο, σε τμήματα αν χρειάζεται. Δεν περιμένει για επιβεβαίωση, παρά μόνο για την ίδια την απάντηση. Το παιδί ελέγχει πρώτα αν πράγματι 
            διαρχειρίζεται την countryTo, αν υπάρχει το ID του πολίτη στον citizen_map και αν αυτός κατάγεται από αυτή την ώρα. Μετά, ελέγχει για τον εμβολιασμό 
            αυτού του πολίτη και απαντά είτε YES vaccinationDate είτε NO. Αν η απάντηση είναι αρνητική, περιμένει για τυχόν επόμενο ερώτημα. Αν όχι, περιμένει 
            να λάβει από τον γονιό απάντηση για το αν η ημερομηνία εμβολιασμού είναι εντός έξι μηνών από την ημερομηνία ταξιδιού.
        3.3) /addVaccinationRecords.
            Ο γονιός στέλνει το SIGUSR1 στο παιδί που διαχειρίζεται την εν λόγω χώρα, και κατόπιν μπλοκάρει περιμένοντας τα ενημερωμένα bloom filters, ακριβώς με
            τον ίδιο τρόπο που περιγράφεται στο 2.2, απλά γνωρίζοντας ότι αναμένει να διαβάσει από συγκεκριμένο παιδί. Το παιδί θα κοιτάξει όλους του φακέλους 
            τους οποίους διαχειρίζεται για νέα αρχεία, και ύστερα θα στείλει στον γονιό τα ενημερωμένα bloom filters.
...
ΓΙΑ ΤΗΝ ΕΝΤΟΛΗ TRAVELSTATS
    Όταν γίνεται μία αίτηση μέσω της εντολής travelRequest, δεν γίνεται έλεγχος για το αν η χώρα countryTo υπάρχει στο directory input_dir ή αν υπάρχει, αλλά
    δεν έχει κάποια γραμμή εμβολιασμού για τον ιό virusName. Θεωρούμε ότι όλες οι χώρες ελέγχουν για όλους τους ιούς. 

ΠΕΡΙΟΡΙΣΜΟΣ ΣΤΟ ΜΕΓΕΘΟΣ ΤΟΥ buffer
    Με εξαίρεση τα μεγέθη: bufferSize, sizeOfBloom και τα μήκη ιών, χωρών και λοιπών συμβολοσειρών, τα οποία αποστέλλονται ως ένας int, τα δεδομένα μπορούν
    να μεταφερθούν ακόμα και byte-byte. Δηλαδή, αν δοθεί όρισμα -b 1, το μήκος ενός directory θα περαστεί ως int, όμως το ίδιο το όνομά του θα περάσει χαρακτήρα-
    χαρακτήρας. Εμμέσως λοιπόν το ελάχιστο bufferSize είναι 4 (sizeof(int)), αλλά 'υβριδικά' μπορεί να λειτουργήσει και με μέγεθος 1.

ΑΠΟΡΡΙΨΗ ΔΙΠΛΟΤΥΠΩΝ
    Η απόρριψη γίνεται ακριβώς με τον ίδιο τρόπο όπως στην πρώτη εργασία: απορρίπτονται εγγραφές οι οποίες έχουν NO και ημερομηνία εμβολιασμού ή YES και δεν έχουν
    ημερομηνία. Επίσης, αν έρθει μία έγκυρη εγγραφή για έναν πολίτη για έναν ιό, τότε οι επόμενες για αυτόν απορρίπτονται. Δηλαδή, αν η πρώτη έγκυρη εγγραφή για
    τον πολίτη 22 και τον ιό COVID-19 έχει NO ως κατάσταση εμβολιασμού, απορρίπτουμε όποια εγγραφή έρθει και πει το αντίθετο. 
...
ΔΙΑΧΕΙΡΙΣΗ ΣΗΜΑΤΩΝ
    ...
    Λήψη Σήματος SIGINT/SIGQUIT από το Monitor process
        ...
        ερωτήματος. Αυτό ισχύει και για το σήμα SIGCHLD που θα λάβει ο γονιός για να αντικαταστήσει ένα παιδί που πέθανε ξαφνικά: θεωρούμε ότι έχει
        ολοκληρωθεί οποιαδήποτε άλλη λειτουργία (αυτό συμπεριλαμβάνει την αντικατάσταση ενός παιδιού που μόλις πέθανε) όταν έρχεται ένα σήμα SIGHCHLD. 
        Αν το σήμα ληφθεί όσο είτε το Monitor process περιμένει ερώτημα από τον γονιό είτε ο γονιός είσοδο από τον χρήστη, τότε ο χειρισμός του σήματος 
        είναι άμεσος.

        Οι handlers θέτουν ένα flag για το ανάλογο σήμα στην τιμή 1, και η διεργασία ελέγχει στις προαναφερθείσες στιγμές (μετά από ένα ερώτημα ή αν λάβει 
        EINTR στην select) την τιμή του flag και προβαίνει στις ανάλογες ενέργειες.

        Αν ένα παιδί λάβει πάνω από μία φορές SIGINT/SIGQUIT, το παλιό log_file.xxx θα αντικαθίσταται με το καινούριο. 

        Η αποστολή του σήματος SIGINT/SIGQUIT στον γονιό και στα παιδιά έχει δοκιμαστεί μέσω του terminal με την εντολή kill -2 process_id και 
        kill -s INT process_id/kill -s QUIT process_id.
...
BASH SCRIPT
    ...
    Αυτός ο πίνακας για κάθε χώρα κρατά τον αριθμό του αρχείου στο οποίο πρόκειται να γράψει την επόμενη εγγραφή, επομένως κάθε στοιχείο του αρχικοποιείται με 0.
